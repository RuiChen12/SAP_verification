...
let check_invariant2 bs name = assert (List.length name <= (Int64.to_int bs) - 9)
...
if nb <= 0L then ()
     ...
     clear_file (Int64.add bn 1L) (Int64.sub nb 1L) zero_block)
in
...
let size = Int64.of_int (8 + (List.length lname) +
                   (List.length data)) in
